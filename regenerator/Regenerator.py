...
class Regenerator:
    ...
    def generate_new_puzzle(self, puzzle: Puzzle) -> Union[List[Puzzle], None]:
        ...
        white_pieces = {'R', 'N', 'B', 'Q', 'P'}
        black_pieces = {'r', 'n', 'b', 'q', 'p'}
        # get the Fen of the puzzle root node
        current_board = puzzle.node.board()

        fen_of_puzzle_origin = puzzle.node.board().fen()
        removable_pieces = fen_of_puzzle_origin.split("/")
        # ['rnbqk1nr', 'pppppppp', '8', '8', '8', '8', 'PPPPPPPP', 'RNBQK1NR']
        for pieces in removable_pieces:
            if pieces == "8":
                continue
            for piece in pieces:
        ...
        # Remove the possible pieces of the original fen
        # TODO:
        fen_of_puzzle_modified = fen_of_puzzle_origin


        new_game = chess.pgn.Game()
        new_board = chess.Board(fen_of_puzzle_modified)
        new_game.setup(new_board)
        prev_score = puzzle.cp


        new_puzzle = self.analyze_position(new_game, prev_score, tier=10)


    def analyze_position(self, node: GameNode, prev_score: Score, current_eval: PovScore, tier: int) -> Union[Puzzle, Score]:
...
def main() -> None:
    board = chess.Board("rnbqkbnr/pp1p2pp/2p2p2/3Pp3/4P3/8/PPP2PPP/RNBQKBNR w KQkq e6 0 4")
